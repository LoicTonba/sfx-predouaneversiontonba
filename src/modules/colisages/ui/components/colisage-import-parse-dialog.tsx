...
interface ColisageImportParseDialogProps {
  open: boolean;
...
export const ColisageImportParseDialog = ({
  open,
  onOpenChange,
  dossierId,
  parsedRows,
  existingRowKeys = [],
}: ColisageImportParseDialogProps) => {
  const [selectedRows, setSelectedRows] = useState<Set<number>>( new Set(parsedRows.map((_, idx) => idx)));
  ...
  const toggleAll = () => {
    if (selectedRows.size === editedRows.length) {
      ...
      setSelectedRows(new Set(editedRows.map((_, index) => index)));
    }
  };
  ...
  const handleImport = async () => {
    ...
    const rowsToImport = editedRows.filter((_, index) => selectedRows.has(index));
  ...
  const newCount = editedRows.filter(
    (row, index) => selectedRows.has(index) && !existingKeysSet.has(row.uploadKey)
  ).length;
                    ...
                    <div className="space-y-2">
                        {editedRows.length === 0 ? (
                            ...
                            editedRows.map((row, idx) => {
                                const isExisting = existingKeysSet.has(row.uploadKey);
